/*
  # Update Push Notification Trigger to Pass Message ID
  
  1. Changes
    - Update the push notification trigger to extract message_id from notification data
    - Pass message_id to the Edge Function when notification type is 'new_message'
    - Edge Function will then mark the message as delivered when push is sent successfully
  
  2. Behavior
    - When a message notification is created, the trigger extracts the message_id
    - The Edge Function receives the message_id and marks the message as delivered
    - This ensures delivered_at is set when push notification is actually sent
*/

-- Update the function to include message_id in the payload
